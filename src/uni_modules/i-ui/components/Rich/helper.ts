...
function HTMLParser(html: string, handler: any) {
    var index;
    var chars;
    var match;
    var stack: any = [];
        ...
        if (!stack.last() || !special[stack.last()]) {
            ...
            if (html.indexOf('<!--') == 0) {
                ...
                if (match) {
                    ...
                    match[0].replace(endTag, parseEndTag as unknown as string);
                    chars = false;
                } // start tag
                ...
                if (match) {
                    html = html.substring(match[0].length);
                    match[0].replace(startTag, parseStartTag as unknown as string);
                    chars = false;
                }
            }
...
function makeMap(str: string) {
    var obj: { [propName : string] : boolean } = {};
    var items: string[] = str.split(',');
...
function parseHtml(html: string) {
    ...
    HTMLParser(html, {
        start: function start(tag?: any, attrs?: any, unary?: any) {
            ...
            var node: any = stacks.shift();
            if (node.name !== tag) console.error('invalid state: mismatch end tag');

            if (stacks.length === 0) {
                results.children.push(node as never);
            } else {
                var parent = stacks[0];
